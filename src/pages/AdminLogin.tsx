  ...
  return (
    <div className="grid min-h-screen place-items-center bg-base px-6 py-10">
      ...
      >
        ...
        {mode === 'login' ? (
          <LoginForm onForgot={() => setMode('forgot')} onSuccess={() => navigate('/admin/produtos')} />
        ) : (
  ...
  return (
    <>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4">
        ...
        <a href="/" className="text-center font-sans text-sm text-muted hover:text-gold-text">
          ← Voltar à vitrine
        </a>
      </form>
    </>
...
function ForgotForm({ onBack }: { onBack: () => void }) {
  ...
  const onSubmit = async (data: ForgotData) => {
    ...
    if (error) {
      setServerError('Não foi possível enviar o link. Tente novamente.')
      return
    }
    setSent(true)
  }
  ...
  if (sent) {
    return (
      <div className="text-center">
        <h1 className="font-display text-2xl font-semibold text-strong">Verifique seu e-mail</h1>
        <p className="mt-3 font-sans text-sm text-body">
          Enviamos um link de recuperação para <strong className="text-strong">{getValues('email')}</strong>.
          Abra o e-mail e clique no link para definir uma nova senha.
        </p>
        <button
          type="button"
          onClick={onBack}
  ...
  return (
    <>
      <div className="mb-6 text-center">
        <h1 className="font-display text-2xl font-semibold text-strong">Recuperar senha</h1>
        <p className="bb-eyebrow mt-2 text-xs">Enviaremos um link por e-mail</p>
      </div>
